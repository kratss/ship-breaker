...
# Get normals
environment_plane_intersection.estimate_normals(
    search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30)
)
print(np.asarray(environment_plane_intersection.normals))
lines = []
for i in range(len(environment_plane_intersection.points)):
    lines.append([i, len(environment_plane_intersection.points) + i])

normal_lines = o3d.geometry.LineSet()
normal_lines.points = o3d.utility.Vector3dVector(
    np.vstack(
        (
            np.asarray(environment_plane_intersection.points),
            np.asarray(environment_plane_intersection.points)
            + np.asarray(environment_plane_intersection.normals),
        )
    )
)
